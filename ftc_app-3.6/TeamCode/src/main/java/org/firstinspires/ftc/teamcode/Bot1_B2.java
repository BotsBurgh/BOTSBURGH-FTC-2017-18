...
@Autonomous(name="Bot1 B2", group ="Concept")

public class Bot1_B2 extends LinearOpMode {

    private com.disnodeteam.dogecv.detectors.JewelDetector jewelDetector = null;
    
    private final double UP_POS = 0.17;
    ...
    @Override public void runOpMode() {
        ...
        this.vuforia = new ClosableVuforiaLocalizer(parameters);        VuforiaTrackables relicTrackables = this.vuforia.loadTrackablesFromAsset("RelicVuMark");
        ...
        encoderDrive(-.3, -14.5,-14.5,7.0);
        sleep(100);
        if (vuMark == RelicRecoveryVuMark.LEFT)
        {
            encoderDrive(DRIVE_SPEED, 12.75, 12.75, 7.0);
        }
        if (vuMark == RelicRecoveryVuMark.CENTER|| vuMark == RelicRecoveryVuMark.UNKNOWN)
        {
            encoderDrive(DRIVE_SPEED, 16.75, 16.75, 8.5);
        }
        if (vuMark == RelicRecoveryVuMark.RIGHT )
        {
            encoderDrive(DRIVE_SPEED, 19.75, 19.75, 10.0);
        }
        sleep(150);
        left.setPower(-.2);
        right.setPower(.06);
        
        while (Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) < 140 && Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) > 0)
        {
            telemetry.update();
        }
        ...
        right.setPower(.05);
        //-176 + 18 inches    
        while (Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) < 160 && Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)) > 0)
        {
            telemetry.update();
        }
